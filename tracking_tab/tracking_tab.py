...
class TrackingTab:
    def __init__(self):
        ...
        self.circle_roi_data = {}  # Dict to store {roi_id: {center_x, center_y, radius}}
    ...
    def _bb_pan_start(self, event):      
        if self.select_roi.value == "Rectangle":  
            ...
            self.roi_count += 1
            self.circle_start_point['x'], self.circle_start_point['y'] = event.x, event.y
            
            # Create temporary circle using line to draw circle outline
            import numpy as np
            angles = np.linspace(0, 2*np.pi, 50)
            circle_x = [event.x + 5*np.cos(angle) for angle in angles]  # Start with radius 5
            circle_y = [event.y + 5*np.sin(angle) for angle in angles]
            
            self.circle_temp_annotation = self.frame_pane.line(circle_x, circle_y,
                                                               line_color='green',
                                                               line_width=2,
                                                               line_dash='dashed',
                                                               line_alpha=0.8)
    ...
    def _bb_pan_end(self, event): 
        if self.video_loaded and self.select_roi.value == "Rectangle":       
            ...
            final_circle = self.frame_pane.scatter([center_x], [center_y], 
                                                  size=size_value,
                                                  line_color='red',
                                                  line_width=2,
                                                  fill_alpha=0.3,
            ...
            print(f"Enabling Start button. Current state: disabled={self.button_start_tracking.disabled}")
            ...
            try:
                ...
                if hasattr(self.button_start_tracking, 'param') and hasattr(self.button_start_tracking.param, 'watch'):
                    self.button_start_tracking.param.disabled = False
                
                print(f"After force update - button disabled: {self.button_start_tracking.disabled}")
            except Exception as e:
            ...
            print(f"Circle ROI created. Total ROIs: {len(self.rois)}, roi_count: {self.roi_count}")
            print(f"video_loaded: {self.video_loaded}, button disabled: {self.button_start_tracking.disabled}")
        ...
        if self.select_roi.value == "Polygon":
            ...
            if distance < 10 and size_poly>1:
                ...
                if size_poly > 1:
                    ...
                    line = self.frame_pane.line([prev_x, x], [prev_y, y], 
                                               line_color="blue", line_width=2, 
                                               line_alpha=0.7, line_dash='dashed')
                    self.poly_lines.append(line)
         
    def _circle_annotation_old(self, event):  # Deprecated - now using drag
        if self.select_roi.value == "Circle": # self.video_loaded and
            ...
            if dots_number < 2:
                ...
                if dots_number==2:
                    ...
                    print(f"🔴 CREATING Circle: original_radius={radius}, size_value={size_value}")
                    circle_draw = self.frame_pane.scatter([x0], [y0], size=size_value, line_color="green", fill_color="green", fill_alpha=0.3, marker='circle')
        ...
        try:
            ...
            for roi in self.rois[:]:
                try:
                    ...
                    if hasattr(self.frame_pane, 'center') and roi in self.frame_pane.center:
            ...
            if hasattr(self.frame_pane, 'center'):
                ...
                for annotation in self.frame_pane.center[:]:
                    ...
                    if 'BoxAnnotation' not in annotation_type and 'PolyAnnotation' not in annotation_type:
            ...
            if hasattr(self, 'background') and self.background is not None:
                ...
                self._add_log_message("Background image restored after ROI clearing", "info")
                ...
                self.current_frame = self.frame_pane.image_rgba(image=[self.frame_view], x=0, y=0, dw=640, dh=640)
                self._add_log_message("Current frame restored after ROI clearing", "info")
        ...
        try:
            if os.path.exists(MODELS_DIR):
                ...
                if not self.models_name:
                    self.models_name = ["No models found - Place .pt files in tracking_tab/models/"]
            else:
    ...
    def _update_select_models(self):
    ...
    def _load_model(self):
        try:
            ...
            self._add_log_message(f"Loading model: {self.select_model_name.value}", "model")
    ...
    def _load_video(self, event):
        ...
        if self.file_input.value is not None:
            ...
            self.tmp_file = os.path.join(TEMP_DIR, "tmp_file" + mime_to_ext[video_format])
    ...
    def _start_tracking(self, event):
        ...
        print(f"Start tracking clicked. ROIs: {len(self.rois)}, video_loaded: {self.video_loaded}")
        print("="*50)
        ...
        if self.yolo_model is not None:
            ...
            self.progress_bar.name = f"Tracking Progress (0 / {self.total_frames} frames)"
            self.progress_bar.visible = True
            self.progress_bar.value = 0
            ...
            if len(self.rois) > 0:
                self.roi_in_track = self.rois.copy()  # Make a copy to avoid reference issues
            else:
            ...
            self._add_log_message(f"Starting tracking with {len(self.roi_in_track) if self.roi_in_track else 0} ROIs", "info")
            self._add_log_message(f"Frame processing rate: ~30 FPS (33ms interval)", "info")
            
            self.callback_tracking = pn.state.add_periodic_callback(
                lambda: self._capture_tracking_frame(self.roi_in_track),
                33)
    
    def _pause_tracking(self, event):
        ...
        if self.tracking_paused:
            ...
            self._add_log_message("▶ Tracking resumed - continuing from where it stopped", "info")
        else:
            # Pause tracking
    ...
    def _stop_tracking(self, event):
        ...
        self.progress_bar.visible = False
        self._add_log_message("🛑 Tracking stopped - Cleaning up current experiment", "info")
        self._cleanup_experiment()
        if self.video_loaded:
            self._add_log_message("🔄 Ready for new experiment with same video", "success")
        else:
            self._add_log_message("📁 Ready for new experiment - please load a video", "info")
    ...
    def _cleanup_experiment(self):
        ...
        if hasattr(self, 'background') and self.background is not None and self.video_loaded:
            ...
            self.frame_pane.renderers = []
            self.frame_pane.image_rgba(image=[self.frame_view], x=0, y=0, dw=640, dh=640)
    ...
    def _capture_tracking_frame(self, rois):
        try:
            ...
            if not ret or self.frame_count >= self.total_frames:
                ...
                self.progress_bar.visible = False
                if not ret:
                    self._add_log_message("✅ Tracking completed successfully!", "success")
                else:
                    self._add_log_message("✅ Tracking completed - reached end of video!", "success")
                return        
    ...
    def _update_tracking_log(self):
        # Main tracking stats
        log_text = f"### 📊 Tracking Status\n\n"
        log_text += f"**Device:** {self.device}\n\n"
        log_text += f"**Total Frames:** {self.frame_count}\n\n"
        log_text += f"**Frames without detection:** {self.no_detection_count}\n\n"
        log_text += f"**YOLO detections:** {self.yolo_detections}\n\n"
        log_text += f"**Template detections:** {self.template_detections}\n\n"
        ...
        if self.roi_in_track and len(self.frame_per_roi) > 0:
            for index, roi in enumerate(self.roi_in_track):
                if index < len(self.frame_per_roi):
                    log_text += f"**Roi {index}:** {self.frame_per_roi[index]} frames\n\n"
        
        if self.tracking_data and len(self.tracking_data) > 0:
            last_frame = self.tracking_data[-1]
            log_text += f"**Last Centroid X:** {last_frame.get('centroid_x', 'N/A')}\n\n"
            log_text += f"**Last Centroid Y:** {last_frame.get('centroid_y', 'N/A')}\n\n"
