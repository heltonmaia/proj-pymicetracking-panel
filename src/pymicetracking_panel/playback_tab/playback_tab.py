...
class PlaybackTab:
    def __init__(self):
        self.dir_select = pn.widgets.TextInput(name='Video folder', value=DEFAULT_DIR, width=400)
        self.refresh_button = pn.widgets.Button(name='🔄 Refresh', button_type='primary', width=100)
        self.video_select = pn.widgets.Select(name='Available videos', options=self._list_videos(DEFAULT_DIR), width=400)
    ...
    def _empty_video_html(self):
        return "<div style='width:640px; height:360px; border:2px dashed #ccc; display:flex; align-items:center; justify-content:center; color:#888;'>No video selected</div>"
