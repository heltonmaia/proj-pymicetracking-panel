...
class TrackingVisualizer:
    def __init__(self, video_path: str, json_path: str, output_path: Optional[str] = None, show_info: bool = False, show_heatmap: bool = True):
        ...
        if output_path:
            ...
            self.output_path = self.video_path.parent / f"{stem}_tracking_visualized{suffix}"
        ...
        self.writer = cv.VideoWriter(
            str(self.output_path), fourcc, self.fps, (self.frame_width, self.frame_height)
        )
        ...
        self.heat_map = np.zeros((self.mini_map_height, self.mini_map_width), dtype=np.float32)
    ...
    def draw_rois(self, frame: np.ndarray, rois: List[Dict]) -> np.ndarray:
        ...
        for roi in rois:
            ...
            if roi_type == 'rectangle':
                ...
                cv.rectangle(frame, (int(x0), int(y0)), (int(x1), int(y1)), 
                           COLORS["roi_rectangle"], 2)
                # Add label
                cv.putText(frame, "ROI", (int(x0), int(y0) - 5), 
                          FONT, FONT_SCALE, COLORS["roi_rectangle"], THICKNESS)
            
            elif roi_type == 'circle':
                center_x, center_y, radius = roi['coordinates']
                cv.circle(frame, (int(center_x), int(center_y)), int(radius), 
                         COLORS["roi_circle"], 2)
                # Add slight fill for better visibility
                overlay = frame.copy()
                cv.circle(overlay, (int(center_x), int(center_y)), int(radius), 
                         COLORS["roi_circle"], -1)
                cv.addWeighted(overlay, 0.1, frame, 0.9, 0, frame)
                # Add label
                cv.putText(frame, "ROI", (int(center_x - radius), int(center_y - radius - 5)), 
                          FONT, FONT_SCALE, COLORS["roi_circle"], THICKNESS)
                ...
                if len(roi['coordinates']) > 0:
                    ...
                    cv.putText(frame, "ROI", (int(x), int(y) - 5), 
                              FONT, FONT_SCALE, COLORS["roi_polygon"], THICKNESS)
        
        return frame
    
    def draw_centroid(self, frame: np.ndarray, x: int, y: int, detection_method: str) -> np.ndarray:
    ...
    def draw_info_panel(self, frame: np.ndarray, frame_data: Dict, frame_num: int) -> np.ndarray:
        ...
        overlay = frame.copy()
        cv.rectangle(overlay, (panel_x, panel_y), (panel_x + panel_width, panel_y + panel_height), 
                    COLORS["info_bg"], -1)
        ...
        # Frame information
        cv.putText(frame, f"Frame: {frame_num}/{self.total_frames}", 
                  (text_x, y_offset), FONT, font_scale, COLORS["text"], 1)
        y_offset += line_height
        
        # Detection method
        detection_method = frame_data.get("detection_method", "None")
        cv.putText(frame, f"Method: {detection_method}", 
                  (text_x, y_offset), FONT, font_scale, COLORS["text"], 1)
        y_offset += line_height
        ...
        if centroid_x is not None and centroid_y is not None:
            cv.putText(frame, f"XY: ({centroid_x}, {centroid_y})", 
                      (text_x, y_offset), FONT, font_scale, COLORS["text"], 1)
        else:
            cv.putText(frame, "XY: No detection", 
                      (text_x, y_offset), FONT, font_scale, COLORS["text"], 1)
        y_offset += line_height
        ...
        cv.putText(frame, f"Y:{total_yolo} T:{total_template}", 
                  (text_x, y_offset), FONT, font_scale, COLORS["text"], 1)
        
        return frame
    
    def update_heat_map(self, centroid_x: int, centroid_y: int):
        ...
        for dy in range(-radius, radius + 1):
            for dx in range(-radius, radius + 1):
                ...
                # Check bounds
                if 0 <= heat_x < self.mini_map_width and 0 <= heat_y < self.mini_map_height:
                    # Calculate distance from center
                    distance = np.sqrt(dx*dx + dy*dy)
                    if distance <= radius:
                        # Gaussian-like heat distribution (higher at center)
                        heat_intensity = np.exp(-(distance**2) / (2 * (radius/2)**2))
    ...
    def draw_mini_map(self, frame: np.ndarray) -> np.ndarray:
        ...
        mini_map_region = np.zeros((self.mini_map_height, self.mini_map_width, 3), dtype=np.uint8)
        
        # Normalize heat map for visualization
        if np.max(self.heat_map) > 0:
            normalized_heat = (self.heat_map / np.max(self.heat_map) * 255).astype(np.uint8)
        ...
        overlay[mini_y:mini_y + self.mini_map_height, 
                mini_x:mini_x + self.mini_map_width] = mini_map_region
        
        # Blend with original frame
        cv.addWeighted(overlay, 0.6, frame, 0.4, 0, frame)
        
        # Draw border
        cv.rectangle(frame, (mini_x - 2, mini_y - 2), 
        ...
        # Add mini-map label
        cv.putText(frame, "Heat Map", (mini_x, mini_y - 10), 
                  FONT, 0.35, COLORS["text"], 1)
    ...
    def process_video(self):
        ...
        print(f"Using tracking data: {self.json_path}")
        ...
        while True:
            ...
            if frame_num % 100 == 0:
                ...
                print(f"Progress: {progress:.1f}% ({frame_num}/{self.total_frames} frames)")
